    ...
    <>
      ...
      {isOpen && selectedPair && (
        <Dialog open={isOpen} onClose={closeModal} className="relative z-50">
          ...
          {/* Контейнер модалки */}
          <div className="fixed inset-0 z-50 flex items-center justify-center border-8 p-4">
            <DialogPanel
              as={motion.div}
              ...
              className="relative z-0 h-[80vh] w-[90vw] max-w-5xl items-center justify-center rounded-lg bg-gray-800/30 shadow-xl backdrop-blur-xl"
            >
              {/* Изображение */}
              {/* <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="relative border-8 border-amber-500 h-90 w-90 inset-0 z-0"
                > */}
              <div className="relative z-0 h-full w-full">
                <AnimatePresence mode="sync">
                  <motion.div
                    key={showBefore ? 'before' : 'after'}
                    initial={{ opacity: 0, x: showBefore ? 4 : -4 }}
                    animate={{ opacity: 1, x: 0 }}
                    exit={{ opacity: 0, x: showBefore ? -4 : 4 }}
                    transition={{ duration: 0.5, ease: 'easeInOut' }}
                    className="absolute inset-0 flex h-full w-full max-w-5xl items-center justify-center"
                  >
                    <Image
                      ...
                      height={900}
                      priority
                      className="h-full w-full rounded-lg object-cover ring-1 ring-gray-500 md:object-[10%_10%]"
                    />
                    {/* крестик закрытия */}
                    <button
                      onClick={closeModal}
